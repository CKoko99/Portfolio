...
const newApps = [
  {
    ...
    tech: ["HTML", "CSS", "Javascript", "React", "Material Ui", "Next Js", "Firebase", "Node", "Express", "Google Cloud Platform"],
  },
  {
    id: "CougarCafe",
    heading: "Cougar Cafe",
    ...
    techOverview: ["The application was built with modern web technologies, utilizing React for the frontend and PHP and MySQL for the backend. The frontend was designed with a single-page application in mind, utilizing React Router to navigate between pages and Material UI for a responsive design that's easy to interact with.",
  ...
  },
  {
    id: "Ck Trips",
    heading: "Ck Trips",
    mobileImages: [CkTMobileImg1, CkTMobileImg2, CkTMobileImg3, CkTMobileImg4, CkTMobileImg5, CkTMobileImg6],
    desktopImages: [CkTDeskImg1, CkTDeskImg2, CkTDeskImg3, CkTDeskImg4, CkTDeskImg5],
    links: [
      {
    ...
    tech: ["HTML", "CSS", "Javascript", "React", "Redux", "Next Js", "Firebase"],
    overview: [
  ...
  },
  {
    ...
    techOverview: ["The application was built with modern web technologies, utilizing React for the user interface, Redux for state management, and Firebase for data storage and authentication. These technologies work together to provide users with a fast, responsive, and reliable experience.",
      ...
      <Box sx={{ ...projectStyles.mainContent }}>
        <Box sx={{ ...projectStyles.halfSection }}>
          ...
          <Box sx={{ ...projectStyles.imgSection }}>
            <KeyboardArrowLeftIcon sx={{ ...projectStyles.arrowButton }}
              onClick={() => { showDesktop ? handleDesktopIndexChange(desktopIndex - 1) : handleMobileIndexChange(mobileIndex - 1) }} />
            <Box
              sx={{ ...projectStyles.imgContainer }}
            >
              {showDesktop && <Box>
                {props.desktopImages?.map((img, index) => (
                  <Box key={index}
                    sx={{ ...projectStyles.desktopImage, display: index === desktopIndex ? "block" : "none" }}
                  >
              ...
              {!showDesktop && <Box>
                {props.mobileImages?.map((img, index) => (
                  <Box key={index}
                    sx={{ ...projectStyles.mobileImage, display: index === mobileIndex ? "block" : "none" }}
                  >
                    <Image alt={props.heading} src={img} />
                  </Box>
                ))}
            ...
            <KeyboardArrowRightIcon sx={{ ...projectStyles.arrowButton }}
              onClick={() => { showDesktop ? handleDesktopIndexChange(desktopIndex + 1) : handleMobileIndexChange(mobileIndex + 1) }}
            />
          </Box>
          <Box>
            {showDesktop && <>{props.desktopImages?.map((img, index) => (
              <>{index !== desktopIndex ? <TripOriginIcon sx={{ ...projectStyles.indexButtons }} key={index} onClick={() => {
                setDesktopIndex(index)
              }} /> :
                <Circle key={index} sx={{ ...projectStyles.indexButtons }} />}
              </>
            ))}</>}
            {!showDesktop && <>{props.mobileImages?.map((img, index) => (
              <>{index !== mobileIndex ? <TripOriginIcon sx={{ ...projectStyles.indexButtons }} key={index} onClick={() => {
                setMobileIndex(index)
              }} /> :
                <Circle key={index} sx={{ ...projectStyles.indexButtons }} />}
              </>
          ...
          <Box sx={{ ...projectStyles.textSection }}>
            <Box>
              {props.links?.map((link, index) => (
                <Button key={index} href={link.href} rel={link.rel} target={link.target}>
                  {link.text}
                </Button>
              ))}
            </Box>
            <Box sx={{ ...projectStyles.technologies }}>
              {props.tech?.map((tech, index) => (
                <Button key={index} disableRipple sx={{ ...projectStyles.techButton }} variant="contained" color="secondary" >{tech}</Button>
              ))}
            </Box>
            <Box sx={{ ...projectStyles.overviewSection }}>
              <Typography variant="subtitle1">{props.overview}</Typography>
              {props.techOverview ? <Button sx={{ ...projectStyles.techOverviewButton }} onClick={() => { setOpenModal(true) }}>Show Overview For Nerds</Button> : null}
              {openModal && <Modal
                open={openModal}
              ...
              >
                <Box sx={{ ...projectStyles.modal }}>
                  ...
                  {props.techOverview?.map((overview, index) => (
                    <Typography key={index} variant="subtitle1">{overview}</Typography>
                  ))}
                </Box>
              </Modal>
              }
  ...
  return (
    <>
      <div className={classes["Portfolio"]}>
        <Typography sx={{ ...styles.heading }} variant="h2">Web Applications</Typography>
        <Typography sx={{ ...styles.headingSubtitle }} variant="subtitle1">
          These are some of the web applications I have designed, and developed using a variety of modern web technologies.
          Projects range from small eccomerce sites to full stack applications with a focus on user experience and performance.
          I strive to build applications that are both powerful and user-friendly to help business become more efficient.
        </Typography>
        <div className={classes["filter"]}>
          <Button variant="contained" color="primary" onClick={() => {
            setOpenFilterModal(true)
          }} >Filter By Tech?</Button>
          {openFilterModal && <Modal
            open={openFilterModal}
            onClose={() => { setOpenFilterModal(false) }}
          >
            <Box sx={{ ...projectStyles.modal }}>
              <Typography align="center" variant="h3">Filter By Tech</Typography>
              <Typography variant="subtitle1">Over the years i've had the pleasure to work with a vast amount of technologies
                to build web applications. Click on a technology to filter the projects below.
              </Typography>
              <Box sx={{ ...projectStyles.technologies }}>
                {technologies.map((tech, index) => {
                  return <Button variant="contained" sx={{ ...projectStyles.techButton, cursor: "pointer" }} key={index} onClick={filterProjects} value={tech}>{tech}</Button>
                })}
              </Box>
            </Box>
          </Modal>}
